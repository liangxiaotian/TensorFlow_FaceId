...
class Net(object):
  ...
  def _variable_on_cpu(self, name, shape, initializer, train=True):
  ...
  def _variable_with_weight_decay(self, name, shape, stddev, wd, train=True):
   ...
   Returns:
    ...
    var = self._variable_on_cpu(name, shape,
      tf.truncated_normal_initializer(stddev=stddev, dtype=tf.float32), train)
    if wd is not None:
      #weight_decay = tf.multiply(tf.nn.l2_loss(var), wd, name='weight_loss')
      weight_decay = tf.contrib.layers.l2_regularizer(wd)(var)
      #加入l2正则化项
      tf.add_to_collection('losses', weight_decay)
    return var 

  def conv2d(self, scope, input, kernel_size, stride=1, train=True, padding = 'SAME'):
    """convolutional layer
    ...
    with tf.variable_scope(scope) as scope:
      kernel = self._variable_with_weight_decay('weights', 
                                      shape=kernel_size,
                                      stddev=5e-2,
                                      wd=self.weight_decay, train=train)
      conv = tf.nn.conv2d(input, kernel, [1, stride, stride, 1], padding=padding)
      biases = self._variable_on_cpu('biases', kernel_size[3:], tf.constant_initializer(0.0), train)
  ...
  def max_pool(self, input, kernel_size, stride, name='pool'):
    """max_pool layer
  ...
  def local(self, scope, input, in_dimension, out_dimension, leaky=True, train=True):
    ...
    with tf.variable_scope(scope) as scope:
      ...
      # reshape = tf.reshape(input, [int(batch_size), -1])
      ...
      weights = self._variable_with_weight_decay('weights', shape=[in_dimension, out_dimension],
                          stddev=0.04, wd=self.weight_decay, train=train)
      biases = self._variable_on_cpu('biases', [out_dimension], tf.constant_initializer(0.0), train)
